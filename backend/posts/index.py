...
def handler(event: dict, context) -> dict:
    ...
    if action == "get_comments":
        ...
        cur.execute(
            f"""
            SELECT c.id, c.text, c.created_at, u.id, u.full_name, u.job_title
        ...
        comments = []
        for r in rows:
            cid, text, created_at, uid, full_name, job_title = r
            initials = "".join(w[0] for w in full_name.split() if w)[:2].upper()
            comments.append({
                "id": cid,
                "text": text,
    ...
    if action == "add_comment":
        ...
        if not post_id or not text:
            return err(400, "Укажите post_id и текст")
        if len(text) > 1000:
            return err(400, "Комментарий слишком длинный")
        ...
        user_id, full_name, job_title = user
        cur.execute(
            f"INSERT INTO {SCHEMA}.comments (post_id, user_id, text) VALUES (%s, %s, %s) RETURNING id, created_at",
        ...
        if row and row[0] != user_id:
            cur.execute(
                f"INSERT INTO {SCHEMA}.notifications (user_id, type, actor_id, post_id, comment_id) VALUES (%s, 'comment', %s, %s, %s)",
                (row[0], user_id, post_id, cid)
            )
