"""
Посты: лента, создание, лайки, комментарии, удаление, просмотры, шары, профиль, админ-панель. v3
"""
...
def handler(event: dict, context) -> dict:
    ...
    if action == "get_followers":
        ...
        cur.execute(f"""SELECT u.id, u.full_name, u.job_title, u.avatar_url,
            EXISTS(SELECT 1 FROM {SCHEMA}.follows WHERE follower_id=%s AND following_id=u.id) as is_following
            FROM {SCHEMA}.follows f JOIN {SCHEMA}.users u ON u.id=f.follower_id
            WHERE f.following_id=%s ORDER BY f.created_at DESC""", (viewer_id, uid))
        rows = cur.fetchall(); conn.close()
        return ok({"users": [{"id": r[0], "full_name": r[1], "job_title": r[2] or "",
            "avatar_url": r[3] or "", "initials": "".join(w[0] for w in r[1].split() if w)[:2].upper(),
            "is_following": bool(r[4])} for r in rows]})
    ...
    if action == "get_following":
        ...
        cur.execute(f"""SELECT u.id, u.full_name, u.job_title, u.avatar_url,
            ...
            WHERE f.follower_id=%s ORDER BY f.created_at DESC""", (viewer_id, uid))
        rows = cur.fetchall(); conn.close()
        return ok({"users": [{"id": r[0], "full_name": r[1], "job_title": r[2] or "",
            "avatar_url": r[3] or "", "initials": "".join(w[0] for w in r[1].split() if w)[:2].upper(),
            "is_following": bool(r[4])} for r in rows]})
